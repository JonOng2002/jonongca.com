...
export const SpotifyWidget: React.FC = () => {
  ...
  return (
    <div
      onClick={openSpotify}
      className={`relative rounded-[2rem] overflow-hidden transition-all duration-300 hover:scale-[1.03] cursor-pointer group glass h-full bg-white dark:bg-obsidian-900 shadow-sm hover:shadow-xl min-h-[300px]
        ${isCurrent ? 'ring-2 ring-transparent hover:ring-green-500' : 'ring-2 ring-transparent hover:ring-blue-500'}
        ${loading ? 'bg-zinc-100 dark:bg-obsidian-900' : ''}
      `}
    >
      {/* Background Layer */}
      {song && (
      ...
      <div className="relative z-10 h-full w-full flex flex-col p-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
            ...
            <p className="text-[10px] font-mono uppercase tracking-widest text-forest/40 dark:text-accent-400/40">Syncing Track...</p>
          </div>
        ) : song ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center opacity-40">
            <svg className="w-8 h-8 text-forest dark:text-accent-500/50 mb-3" fill="currentColor" viewBox="0 0 24 24">
            ...
            <p className="text-[10px] font-mono uppercase tracking-widest text-forest dark:text-accent-300">Playlist Idle</p>
          </div>
        )}
      </div>
    </div>
